...
do_install() {
    install -d ${D}${prefix}/local/demo-ai/computer-vision/models/movenet
    install -d ${D}${prefix}/local/demo-ai/computer-vision/models/movenet/testdata

    #the model is fetched with a bad name -> rename it before installation
    mv ${S}/movenet_singlepose_lightning_quant/4\?lite-format\=tflite ${S}/movenet_singlepose_lightning_quant/lite-model_movenet_singlepose_lightning_tflite_int8_4.tflite
    # install movenet model
    install -m 0644 ${S}/movenet_singlepose_lightning_quant/*.tflite		${D}${prefix}/local/demo-ai/computer-vision/models/movenet/
}
